let books = [
    ...
    "The Three Fundamental Principles # Muhammad Ibn Sulayman At Tameemee # Aboo Shaybah # English, Arabic # Islam # https://qaryah.files.wordpress.com/2011/11/class_3usool.pdf # https://www.sifatusafwa.com/15486/cart_default.jpg",
    ...
    "Sahih Al Bukhari # Imam Al Bukhari # Muhammad Muhsin Khan # English, Arabic # Islam, Hadeeth # https://www.muslim-library.com/dl/books/English_The_Translation_of_the_Meanings_of_Summarized_Sahih_Al_Bukhari.pdf # https://archive.org/services/img/SummarizedSahihAl-bukhari.pdf/full/pct:200/0/default.jpg",
    ...
    "Riyad As Saliheen # Imam An Nawawi # Dar Al Manarah # English, Arabic # Islamic, Hadeeth # https://masjidtaqwa.org/pdfs/Riyad-us-Saliheen.pdf # http://cdn.shopify.com/s/files/1/0075/3290/4561/products/Riyad_Us_Saliheen__2_Vol.__dsbooks_australia_darussalam_islamicbookstore.jpg?v=1619087522",
...
function createBook() {
    books.forEach(book => {
        let info = book.split("#");
        let queryInfo = book.split("https")[0];

        bookshelf.innerHTML +=
        `
        <div class="book-box" data-info="${queryInfo}">
            <div class="image-div">
                <img src="${info[6]}">
            </div>
...
function checkAuthors(info) {
    if (info !== " N/A ") return `<p>${info}</p>`;
    else return "";
}

function checkTranslators(info) {
    if (info !== " N/A ") return `<p>Translated by ${info}</p>`;
    else return "";
}
...
function openFile(file) {
    let url = window.location.toString().replace("index.html", `pdf-viewer.html#${file}`);
